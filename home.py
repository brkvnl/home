...
logging.info('Boiler is', boilerState)
bf.close()

wf=open ('waterfile.txt', 'r')
wf.seek(-1, 2)
waterState=wf.read(1)
logging.info('Watering is ', waterState)
wf.close()

lf=open ('lampfile.txt', 'r')
lf.seek(-1, 2)
lampState = lf.read(1)
logging.info('Lamp is ', lampState)
lf.close()

yf=open ('SICAKSUfile.txt', 'r')
yf.seek(-1, 2)
SICAKSUState=yf.read(1)
logging.info('SICAKSU is ', SICAKSUState)
yf.close()

hf=open ('hidrofile.txt', 'r')
hf.seek(-1, 2)
hidroState=hf.read(1)
logging.info('HIDROFOR is ', hidroState)
hf.close()
...
#***************************************
def boilerON():
    logging.info("BOILER button pressed")
    ...
    if GPIO.input(12) :
        ...
        logging.info(time1, 'BOILER OFF')
        bf.close()

def rainbirdON():
    logging.info("RAINBIRD button pressed")
    wf=open ('waterfile.txt', 'w')
	if GPIO.input(11) :
 		GPIO.output(11,GPIO.LOW)
		wf.write('1')
		waterButton["text"] = "SULAMA ON"
        logging.info(time1, 'WATER ON')
        wf.close()
	else:
...
def lampON():
    logging.info("LAMP button pressed")
    logging.info(int(int(time.strftime('%M'))%10)%2)
	lf=open ('lampfile.txt', 'w')
	if GPIO.input(13) :
 		GPIO.output(13,GPIO.LOW)
		lf.write('1')
		lampButton["text"] = "LAMBA ON"
        logging.info(time1, 'LAMP ON')
		lf.close()
	else:
		GPIO.output(13, GPIO.HIGH)
		lf.write('0'
		lampButton["text"] = "LAMBA OFF"
        logging.info(time1, 'LAMP OFF')
		lf.close()

def SICAKSUON():
    logging.info("SICAKSU button pressed")
	...
	if GPIO.input(15) :
        ...
        logging.info(time1, 'SICAKSU ON')
        yf.close()
	else:
		GPIO.output(15, GPIO.HIGH)
		yf.write('0')
		SICAKSUButton["text"] = "SICAK SU OFF"
        logging.info(time1, 'SICAKSU OFF')
		yf.close()

def hidroforON():
    logging.info("HIDROFOR button pressed")
	hf=open ('hidrofile.txt', 'w')
	if GPIO.input(33) :
 		GPIO.output(33,GPIO.LOW)
		hf.write('1')
		hidroButton["text"] = "HIDRO ON"
        logging.info(time1, 'HIDROFOR ON')
        hf.close()
	else:
		GPIO.output(33, GPIO.HIGH)
		hf.write('0')
		hidroButton["text"] = "HIDRO OFF"
        logging.info(time1, 'HIDROFOR OFF')
		hf.close()
